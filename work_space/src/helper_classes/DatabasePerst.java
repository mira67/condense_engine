...
/* DatabasePerst
 * 
 * A shell for accessing the Perst database. 
 */

public class DatabasePerst extends Database {
	
	///Connection conn;
	
	// For Hibernate usage. Does nothing.
	private DatabasePerst() {}

	public DatabasePerst(String name) {
		this();
		dbName = name;
	...
	public void connect() {
		...
		// Open the database.
		try {
	        ///Class.forName("org.Perst.Driver");
	        ///conn = DriverManager.getConnection("jdbc:Perst:~/"+dbName, "sa", "");
		} catch(Exception e) {
			Tools.errorMessage("DatabasePerst", "connect", "Could not connect with database " + dbName, e);
		}
    ...
    public void connectReadOnly() {
		try {
	        ///Class.forName("org.Perst.Driver");
	        ///conn = DriverManager.getConnection("jdbc:Perst:~/"+dbName+";IFEXISTS=TRUE", "sa", "");
		} catch(Exception e) {
			Tools.errorMessage("DatabasePerst", "connectReadOnly", "Could not connect with database " + dbName, e);
		}
    ...
    protected void writeCheck(String methodName) {
    	if (status != Status.CONNECTED) {
			Tools.errorMessage("DatabasePerst", methodName, 
	...
	public ArrayList<GriddedVector> getVectors( int first, int last ) {
		...
		if (first > last || first < 0) {
			Tools.warningMessage("DatabasePerst::getVectorsInTimeRange: Warning: " +
