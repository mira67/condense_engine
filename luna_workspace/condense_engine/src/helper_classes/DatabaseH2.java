...
public class DatabaseH2 extends Database {
	...
	public void connect() {
		...
		try {
	        ...
	        conn = DriverManager.getConnection("jdbc:h2:tcp://localhost/~/storage/"+dbName+"Raw"+";DB_CLOSE_DELAY=-1", "sa", null);//jdbc:h2:mem:db1
    ...
    public void connectReadOnly() {
		try {
	        Class.forName("org.h2.Driver");
	        conn = DriverManager.getConnection("jdbc:h2:tcp://localhost/~/storage/"+dbName+"Raw"+";IFEXISTS=TRUE", "sa", null);//
	        //in-memory db test
	        //conn = DriverManager.getConnection("jdbc:h2:tcp://10.240.210.131:9292/mem:~/"+dbName+ "Test"+";DB_CLOSE_DELAY=-1", null, null);//DB_CLOSE_DELAY=-1
		} catch(Exception e) {
