...
/* DatabaseBlank
 * 
 * A shell for accessing the Blank database. 
 */

public class DatabaseBlank extends Database {
	
	///Connection conn;
	
	// For Hibernate usage. Does nothing.
	private DatabaseBlank() {}

	public DatabaseBlank(String name) {
		this();
		dbName = name;
	...
	public void connect() {
		...
		// Open the database.
		try {
	        ///Class.forName("org.Blank.Driver");
	        ///conn = DriverManager.getConnection("jdbc:Blank:~/"+dbName, "sa", "");
		} catch(Exception e) {
			Tools.errorMessage("DatabaseBlank", "connect", "Could not connect with database " + dbName, e);
		}
    ...
    public void connectReadOnly() {
		try {
	        //Class.forName("org.Blank.Driver");
	        //conn = DriverManager.getConnection("jdbc:Blank:~/"+dbName+";IFEXISTS=TRUE", "sa", "");
		} catch(Exception e) {
			Tools.errorMessage("DatabaseBlank", "connectReadOnly", "Could not connect with database " + dbName, e);
		}
    ...
    protected void writeCheck(String methodName) {
    	if (status != Status.CONNECTED) {
			Tools.errorMessage("DatabaseBlank", methodName, 
	...
	public ArrayList<GriddedVector> getVectors( int first, int last ) {
		...
		if (first > last || first < 0) {
			Tools.warningMessage("DatabaseBlank::getVectorsInTimeRange: Warning: " +
