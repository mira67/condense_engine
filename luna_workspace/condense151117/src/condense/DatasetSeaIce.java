...
public class DatasetSeaIce extends Dataset {
	 ...
	 */
	public static GriddedLocation[][] getLocations(String filename) {
		...
		try {
			...
			// TODO: the filename being passed is should actually be a path,
			// and we should determine the filename in here.
			...
			try {
				...
				Tools.warningMessage("DatasetSeaIce::getLocations: error during read. File: "
						+ filename);
			}
		} catch (Exception error) {
			Tools.warningMessage("DatasetSeaIce::getLocations: Could not open data file: "
					+ filename);
		}
		...
		try {
			...
			Tools.warningMessage("DatasetSeaIce::getLocations: error during file close: "			
					+ filename);
		}
	 ...
	 */
	public short[][] readData(String filename, GriddedLocation[][] locs, int timestampID) throws Exception {
		...
		try {
			...
			for (int r = 0; r < rows; r++) {
				for (int c = 0; c < cols; c++) {
					// Sea ice coverage goes from 0 to %100. 
					data[r][c] = netcdfData.get(0, r, c);
				}
			}
		} catch (Exception e) {
